...
def ready_data(path, table, table_type):
    if table == "sina":
        ...
        sina_data = sql.queryall(
            "select number, positive_number, negative_number, word_count, pn_word_count, post_content_txt, order_pos, "
            "order_neg, order_nou from sina where " + table_type + "_status = 0 and order_" + table_type + " is not "
                                                                                                           "null")
        ...
        for element in sina_data:
            ...
            file_util.write_file(os.path.join(path, table, table + "_" + table_type,
                                              str(element.get(str('order_' + table_type))) + "-" + element.get(
                                                  'number') + ".txt"), element.get('post_content_txt'))
            write_file(os.path.join(path, table, table + "_" + table_type + '_attitude_word',
                                    str(element.get(str('order_' + table_type))) + "-" + element.get(
                                        'number') + ".txt"), eval(element.get('pn_word_count')))
            count += 1
        finally_result.append(('sina ' + table_type + ' total word count', sina_data_count))
        print("[{}]--write {}_{} data  end......".format(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), table,
                                                         table_type))
    ...
    if table == "sohu":
        ...
        sohu_data = sql.queryall(
            "select number, positive_number, negative_number, word_count, pn_word_count, article_content_txt, "
            "order_pos, order_neg, order_nou from sohu where " + table_type + "_status = 0 and order_" + table_type +
            " is not null")
        ...
        for element in sohu_data:
            ...
            file_util.write_file(os.path.join(path, table, table + "_" + table_type,
                                              str(element.get('order_' + table_type)) + "-" + element.get(
                                                  'number') + ".txt"), element.get('article_content_txt'))
            write_file(os.path.join(path, table, table + "_" + table_type + '_attitude_word',
                                    str(element.get(str('order_' + table_type))) + "-" + element.get(
                                        'number') + ".txt"), eval(element.get('pn_word_count')))
            count += 1
    ...
    if table == "tianya":
        ...
        tianya_data = sql.queryall(
            "select number, positive_number, negative_number, word_count, pn_word_count, question_detail, order_pos, "
            "order_neg, order_nou from tianya where " + table_type + "_status = 0 and order_" + table_type + " is not "
                                                                                                             "null")
        ...
        for element in tianya_data:
            ...
            file_util.write_file(os.path.join(path, table, table + "_" + table_type,
                                              str(element.get('order_' + table_type)) + "-" + element.get(
                                                  'number') + ".txt"), element.get('question_detail'))
            write_file(os.path.join(path, table, table + "_" + table_type + '_attitude_word',
                                    str(element.get(str('order_' + table_type))) + "-" + element.get(
                                        'number') + ".txt"), eval(element.get('pn_word_count')))
            count += 1
...
def export_excel(path, tabel_name, table_type):
    result_data = sql.queryall(
        "select number, positive_number, negative_number, word_count, order_pos, order_neg, order_nou from " + tabel_name + " where " + table_type + "_status = 0 and order_" + table_type + " is not null order by order_" + table_type)
