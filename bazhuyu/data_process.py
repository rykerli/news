...
def get_all_data():
    # start = time.time()
    title_lst = sql.queryall("select title from bzy_origin_data GROUP BY title")
    ...
    for title in title_lst:
        array1 = []
        lst = sql.queryall("select * from bzy_origin_data where title = %s", title)
...
def insert_data(article, comment):
    sql_str1 = "truncate table bzy_article"
    sql_str2 = "truncate table bzy_comment"
    sql.execute(sql_str1)
    sql.execute(sql_str2)
    sql_article = "insert into bzy_article(id, title, time, content, read_num) " \
                  "values (%s, %s, %s, %s, %s)"
    sql_comment = "insert into bzy_comment(user, comment, comment_time, like_num, article_id, id) " \
                  "values (%s, %s, %s, %s, %s, %s)"
