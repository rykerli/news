...
def start(path, data, cateory, step=1000):
    temp_list = [data[i:i + step] for i in range(0, len(data), step)]
    thr_list = [Thread(i, temp_list[i], path, cateory) for i in range(len(temp_list))]
...
def start1(path, data, cateory, step=1000):
    temp_list = [data[i:i + step] for i in range(0, len(data), step)]
    thr_list = [Thread1(i, temp_list[i], path, cateory) for i in range(len(temp_list))]
    [thr.start() for thr in thr_list]
    [thr.join() for thr in thr_list]
...
def get_sort_data():
    ...
    sina_positive_data = sql.queryall("select number, word_count, positive_number, negative_number from sina order by positive_number DESC")
    sina_negative_data = sql.queryall("select number, word_count, positive_number, negative_number from sina order by negative_number DESC")
    ...
    sohu_positive_data = sql.queryall("select number, word_count, positive_number, negative_number from sohu order by positive_number DESC")
    sohu_negative_data = sql.queryall("select number, word_count, positive_number, negative_number from sohu order by negative_number DESC")
    ...
    tianya_positive_data = sql.queryall("select number, word_count, positive_number, negative_number from tianya order by positive_number DESC")
    tianya_negative_data = sql.queryall("select number, word_count, positive_number, negative_number from tianya order by negative_number DESC")
...
#     dir_util.mkdir([os.path.join(path, 'sina_result'), os.path.join(path, 'sohu_result'), os.path.join(path, 'tianya_result')])
