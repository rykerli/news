    ...
    def run(self):
        ...
        for element in self._lst:
            ...
            if self._path == "sina":
                ...
                # 将带否定词的词语置为0(改为-1)
                remove_repeat = RemoveNegatives(element.get('post_content_txt'), list(count_result.keys()),
                                                count_result)
                ...
                sql.execute("update " + self._path + " set pn_word_count = %s where id = %s",
                            (str(sorted_dict), element.get('id')))
            elif self._path == "sohu":
                words = WordCount(element.get('article_content_txt'), {})
                count_result = words.count_word()
                # 将带否定词的词语置为0(改为-1)
                remove_repeat = RemoveNegatives(element.get('article_content_txt'), list(count_result.keys()),
                                                count_result)
                finally_result = remove_repeat.remove_repeat_count()
                sorted_dict = sorted(finally_result.items(), key=lambda item: item[1], reverse=True)
                sql.execute("update " + self._path + " set pn_word_count = %s where id = %s",
...
class Thread1(threading.Thread):
    ...
    def run(self):
        if self._path == "sina":
            print("[{}]--start update sina positive_negative_number, process {} thread......".format(
                time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), self._num))
            for element in self._lst:
            ...
            print("[{}]--end update sina positive_negative_number, process {} thread......".format(
                time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), self._num))
        elif self._path == "sohu":
            print("[{}]--start update sohu positive_negative_number, process {} thread......".format(
                time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), self._num))
            for element in self._lst:
                temp = eval(element.get('pn_word_count'))
                ...
                sql.execute("update sohu set positive_number = %s, negative_number = %s where id = %s",
                            (pos_count, neg_count, element.get('id')))
            print("[{}]--end update sohu positive_negative_number, process {} thread......".format(
                time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), self._num))
        else:
            print("[{}]--start update tianya positive_negative_number, process {} thread......".format(
                time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), self._num))
            for element in self._lst:
                temp = eval(element.get('pn_word_count'))
                ...
                sql.execute("update tianya set positive_number = %s, negative_number = %s where id = %s",
                            (pos_count, neg_count, element.get('id')))
            print("[{}]--end update tianya positive_negative_number, process {} thread......".format(
                time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), self._num))
...
def start1(path, data, step=5000):
    temp_list = [data[i:i + step] for i in range(0, len(data), step)]
    thr_list = [Thread1(i, temp_list[i], path) for i in range(len(temp_list))]
    [thr.start() for thr in thr_list]
    [thr.join() for thr in thr_list]
...
def get_pn_data():
    ...
    print("[{}]--start get sohu sql......".format(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())))
    sohu_data = sql.queryall("select id, pn_word_count from sohu")
    print("[{}]--process sohu sql data end......".format(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())))

    print("[{}]--start get tianya sql......".format(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())))
    tianya_data = sql.queryall("select id, pn_word_count from tianya")
...
def start_main():
    ...
    for element in origin_pn_data:
        print("[{}]--update count {} data  start......".format(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()),
                                                               result_pn_list[i]))
        start1(result_pn_list[i], origin_pn_data[i])
