...
def writeListData1(data, name, table_type):
    temp = [('file name', 'positive number', 'negative number', 'word count', str('order ' + table_type))]
    for element in data:
        temp.append((element.get('number'), element.get('positive_number'), element.get('negative_number'), element.get('word_count'), element.get("order_" + table_type)))
    with open(name, 'a', errors='ignore', newline='') as f:
        f_csv = csv.writer(f)
        f_csv.writerows(temp)
    # print('write_csv success')
